...
- 知识点标签必须来自学科知识图谱
- 分类结果必须经用户确认后才保存
- **structured 和 classification 字段不允许为 null**——调用 save_wrong_question 时必须包含完整的 structured（含学科、年级段、知识点、难度、题型）和 classification（含错误类型、细分类别）
- 若 AI 解析未能完成，必须用默认值填充（学科="数学"待确认、难度="中等"待确认、错误类型="知识漏洞"待确认），并明确告知用户需要修改
